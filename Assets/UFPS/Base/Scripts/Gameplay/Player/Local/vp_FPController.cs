...
public class vp_FPController : vp_Component
{
	...
	private CharacterController m_CharacterController = null;	// should never be referenced directly
	public CharacterController CharacterController	// lazy initialization of the character controller field
	{
		get
		{
	...
	protected virtual void FixedMove()
	{
		...
		Physics.SphereCast(new Ray(Transform.position + Vector3.up * (Player.Radius.Get()), Vector3.down),
									(Player.Radius.Get()), out m_GroundHit, (m_SkinWidth + 0.001f),
		...
		if (!m_Grounded && (Player.Velocity.Get().y > 0.0f))
		{
			Physics.SphereCast(new Ray(Transform.position, Vector3.up),
										...
										Player.Height.Get() - (Player.Radius.Get() - m_SkinWidth) + 0.01f,
										vp_Layer.Mask.ExternalBlockers);
	...
	protected virtual void SmoothMove()
	{
		...
		if (Vector3.Distance(Transform.position, m_SmoothPosition) > Player.Radius.Get())
			m_SmoothPosition = Transform.position;
	...
	/// returns the current slopeLimit
	/// </summary>
	protected virtual float OnValue_Radius
	{
	...
	/// returns the current slopeLimit
	/// </summary>
	protected virtual float OnValue_Height
	{
