...
public class vp_FPWeaponMeleeAttack : vp_Component
{
	...
	void SpawnImpactFX(RaycastHit hit)
	{
		...
		if (m_DustPrefab != null)
			vp_Utility.Instantiate(m_DustPrefab, hit.point, hitNormal);

		// spawn spark effect
		if (m_SparkPrefab != null)
		{
			if (Random.value < SparkFactor)
				vp_Utility.Instantiate(m_SparkPrefab, hit.point, hitNormal);
		}

		// spawn debris particle fx
		if (m_DebrisPrefab != null)
			vp_Utility.Instantiate(m_DebrisPrefab, hit.point, hitNormal);
	...
	void ApplyDamage(RaycastHit hit)
	{
		...
		if (body != null && !body.isKinematic)
			body.AddForceAtPosition(((m_Camera.Transform.forward * DamageForce) / Time.timeScale) / vp_TimeUtility.AdjustedTimeScale, hit.point);
	...
	void DebugDrawImpact(RaycastHit hit)
	{
		...
		for (int v = 0; v < 5; v++)
		{
			...
			pathBall[v].GetComponent<Collider>().enabled = false;
