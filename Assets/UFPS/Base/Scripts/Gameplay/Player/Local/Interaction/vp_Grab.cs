...
public class vp_Grab : vp_Interactable
{
	...
	protected virtual void StopGrab()
	{
		...
		if (vp_Input.GetButtonDown("Attack"))
		{
			...
			if (m_Transform.GetComponent<Rigidbody>() != null)
			{
				m_Transform.GetComponent<Rigidbody>().AddForce(m_Player.Velocity.Get() + m_Player.Forward.Get() * ThrowStrength, ForceMode.Impulse);
		...
		else	// otherwise perform a normal drop
		{
			...
			if (m_Transform.GetComponent<Rigidbody>() != null)
				m_Transform.GetComponent<Rigidbody>().AddForce(m_Player.Velocity.Get() + m_Player.Forward.Get(), ForceMode.Impulse);
