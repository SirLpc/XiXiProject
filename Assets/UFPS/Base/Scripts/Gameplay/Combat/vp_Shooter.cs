...
public class vp_Shooter : vp_Component
{
	...
	protected virtual void EjectShell()
	{
		...
		// send it flying
		if (s.GetComponent<Rigidbody>())
		{

			Vector3 force = (transform.TransformDirection(ShellEjectDirection) * ShellEjectVelocity);
			s.GetComponent<Rigidbody>().AddForce(force, ForceMode.Impulse);
		...
		if (m_CharacterController)
		{
			...
			s.GetComponent<Rigidbody>().AddForce(velocityForce, ForceMode.VelocityChange);
		...
		if (ShellEjectSpin > 0.0f)
		{
			if (Random.value > 0.5f)
				s.GetComponent<Rigidbody>().AddRelativeTorque(-Random.rotation.eulerAngles * ShellEjectSpin);
			else
				s.GetComponent<Rigidbody>().AddRelativeTorque(Random.rotation.eulerAngles * ShellEjectSpin);
		}
